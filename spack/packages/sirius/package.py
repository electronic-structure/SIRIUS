...
class Sirius(CMakePackage, CudaPackage):
    ...
    depends_on('spla@1.2.0:', when='@develop')
    depends_on('spla@1.2.0:+cuda', when='@develop+cuda')
    depends_on('spla@1.2.0:+rocm', when='@develop+rocm')
        ...
        if '+cuda' in spec:
            ...
            if cuda_arch[0] != 'none':
                ...
                # Make SIRIUS handle it
                    ...
                    args.append('-DCUDA_ARCH={0}'.format(';'.join(cuda_arch)))
