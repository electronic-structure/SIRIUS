...
For better performance with Jython, the Java native RE library
is called.
"""
...
class Scanner:
    ...
    """
    # Use native java regexes with Jython

    using_java_regex = False
    if sys.platform.startswith('java'):
        import java.util.regex.Pattern as re
        using_java_regex = True
    else:
        import re
    ...
    def interp_scan(self, restrict):
		...
		if not self.using_java_regex:
                    m = regexp.match(self.input, self.pos)
                else:
                    m = regexp.region(self.pos,len(self.input))
                    t = m.lookingAt()
                    if not t: continue
                    m = m.toMatchResult()
