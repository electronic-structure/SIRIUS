                   ...
                   int bs__)
{
    ...
    Wave_functions phi(pu, gvec, 2 * num_bands__, 2);
    wave_functions tmp(pu, gvec, 2 * num_bands__);

    phi.component(0).pw_coeffs().prime() = [](int64_t i0, int64_t i1){return type_wrapper<double_complex>::random();};
    phi.component(1).pw_coeffs().prime() = [](int64_t i0, int64_t i1){return type_wrapper<double_complex>::random();};
    ...
    if (pu == GPU) {
        for (int is = 0; is < 2; is++) {
            ...
            phi.component(is).pw_coeffs().copy_to_device(0, 2 * num_bands__);
        }
    ...
    orthogonalize<double_complex>(pu, 2, 0, num_bands__, phi, ovlp, tmp);
    orthogonalize<double_complex>(pu, 2, num_bands__, num_bands__, phi, ovlp, tmp);

    inner(2, phi, 0, 2 * num_bands__, phi, 0, 2 * num_bands__, ovlp, 0, 0);
