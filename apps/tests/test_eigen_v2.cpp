               ...
               Eigensolver& solver)
{
    ...
    if (test_gen__) {
        /* lambda * B * Z */
        linalg2(linalg_t::scalapack).gemm('N', 'N', n__, nev__, n__, &linalg_const<T>::one(), B_ref, 0, 0, A, 0, 0,
            &linalg_const<T>::zero(), B, 0, 0);
        B >> A;
    }

    /* A * Z - lambda * B * Z */
    linalg2(linalg_t::scalapack).gemm('N', 'N', n__, nev__, n__, &linalg_const<T>::one(), A_ref, 0, 0, Z, 0, 0,
