...
template <typename T, typename F>
void test_wf_ortho_new(BLACS_grid const& blacs_grid__, double cutoff__, int num_bands__,
                   int bs__, int num_mag_dims__, memory_t mem__, linalg_t la__)
{
...
void call_test(std::vector<int> mpi_grid_dims__, double cutoff__, int num_bands__, int bs__,
               int num_mag_dims__, memory_t mem__, linalg_t la__, int repeat__)
{
    ...
    for (int i = 0; i < repeat__; i++) {
        ...
        test_wf_ortho_new<T, std::complex<double>>(*blacs_grid, cutoff__, num_bands__, bs__, 3, mem__, la__);
    }
}
