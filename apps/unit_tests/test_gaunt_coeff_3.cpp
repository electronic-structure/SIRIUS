                ...
                for (int m2 = -l2; m2 <= l2; m2++) {
                    for (int l3 = 0; l3 <= 8; l3++) {
                        for (int m3 = -l3; m3 <= l3; m3++) {
                            ...
                            for (int i = 0; i < sht.num_points(); i++) {
                                s += std::conj(ylm(sf::lm(l1, m1), i)) * rlm(sf::lm(l2, m2), i) *
                                     ylm(sf::lm(l3, m3), i) * sht.weight(i);
                            }
                            s *= fourpi;
                            d += std::abs(s - SHT::gaunt_yry(l1, l2, l3, m1, m2, m3));
                        }
                    }
                }
            }
...
{
    return call_test(argv[0], test_gaunt_yry_numerical);
}
