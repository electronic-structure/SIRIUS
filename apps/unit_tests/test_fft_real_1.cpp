...
run_test(cmd_args& args, device_t pu__)
{
    ...
    ////spfft::Transform spfft(spfft_grid.create_transform(SPFFT_PU_HOST, fft_type, fft.size(0), fft.size(1),
    ///fft.size(2), /    fft.local_size_z(), gvecp.gvec_count_fft(), SPFFT_INDEX_TRIPLETS, /
    ///gvecp.gvec_coord().at(memory_t::host)));

    // spfft::Transform spfft_r(spfft_grid.create_transform(SPFFT_PU_HOST, fft_type, fft.size(0), fft.size(1),
    // fft.size(2),
