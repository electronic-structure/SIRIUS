...
ENV SPEC="sirius@develop%gcc build_type=Release +fortran +elpa +tests +scalapack +cuda cuda_arch=${CUDA_ARCH} ^mpich@${MPICH_VERSION} ^intel-oneapi-mkl+cluster ^spla ^spfft+cuda cuda_arch=${CUDA_ARCH} ^elpa+cuda cuda_arch=${CUDA_ARCH}"
