...
(
    ...
    srun -n4 -c2 --unbuffered --hint=nomultithread --mpi=pmi2 sarus run --mount=type=bind,source=$SCRATCH,destination=$SCRATCH $SARUS_IMAGE bash -c 'cd $SCRATCH/SIRIUS/verification/test01 && sirius.scf --test_against=output_ref.json --control.std_evp_solver_name=scalapack --control.gen_evp_solver_name=scalapack --control.mpi_grid_dims=2:2 --control.processing_unit=gpu'
