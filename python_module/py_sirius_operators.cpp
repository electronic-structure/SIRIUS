...
void init_operators(py::module& m)
{
    ...
    py::class_<S_k<complex_double>>(m, "S_k")
        .def(py::init<const Simulation_context&, const Q_operator<PT>&, const Beta_projectors_base<PT>&, int>(),
    ...
    py::class_<Ultrasoft_preconditioner<complex_double>>(m, "Precond_us")
        .def(py::init<Simulation_context&, const Q_operator<PT>&, int, const Beta_projectors_base<PT>&, const Gvec&>(),
