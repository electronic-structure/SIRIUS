                  ...
                  wf::Wave_functions<double>& wf, std::shared_ptr<wf::Wave_functions<double>>& swf)
{
    ...
    for (int ispn_step = 0; ispn_step < ctx.num_spinors(); ispn_step++) {
        ...
        if constexpr (gamma_point) {
            H.apply_h_s<double>(ctx.processing_unit_memory_t(), spin_range, wf::band_range(N, N + n), wf, &wf_out,
                                swf.get());
        } else {
            H.apply_h_s<complex_double>(ctx.processing_unit_memory_t(), spin_range, wf::band_range(N, N + n), wf,
                                        &wf_out, swf.get());
        }
    }
...
PYBIND11_MODULE(py_sirius, m)
{
    ...
    py::class_<Simulation_context>(m, "Simulation_context")
            ...
            .def("hubbard_correction", py::overload_cast<>(&Simulation_context::hubbard_correction, py::const_))
            .def("hubbard_correction", py::overload_cast<bool>(&Simulation_context::hubbard_correction))
