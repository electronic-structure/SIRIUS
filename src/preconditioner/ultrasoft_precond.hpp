...
class DiagonalPreconditioner
{
  public:
    ...
    sddk::mdarray<numeric_t, 2> apply(const sddk::mdarray<numeric_t, 2>& X, sddk::device_t processing_unit);
    void apply(sddk::mdarray<numeric_t, 2>& Y, const sddk::mdarray<numeric_t, 2>& X, sddk::device_t processing_unit);
...
DiagonalPreconditioner<numeric_t>::apply(const sddk::mdarray<numeric_t, 2>& X, sddk::device_t processing_unit)
{
...
{
  public:
    Teter(Simulation_context& ctx, const sddk::Gvec& gkvec)
        ...
        , local::OperatorBase(gkvec.count())
    {
        ...
        for (int i = 0; i < gkvec.count(); ++i) {
            // teter formula
            double T  = gkvec.gkvec_cart<sddk::index_domain_t::local>(i).length2();
            double T2 = T * T;
            double T3 = T2 * T;
            double T4 = T2 * T2;
...
class Ultrasoft_preconditioner : public local::OperatorBase
{
  public:
    Ultrasoft_preconditioner(Simulation_context& simulation_context, const Q_operator<double>& q_op, int ispn,
                             const Beta_projectors_base<double>& bp, const sddk::Gvec& gkvec);
    ...
    , bp(bp)
{
    ...
    sddk::linalg(sddk::linalg_t::blas)
        .axpy(n, &sddk::linalg_const<double_complex>::one(), ones.data(), 1, CQ.at(sddk::memory_t::host), n + 1);
    // compute LU factorization
    this->LU = sddk::empty_like(CQ);
    sddk::copy_auto(this->LU, CQ);
...
{
    auto Y = empty_like(X, ctx_.mem_pool(pm == sddk::memory_t::none ? ctx_.preferred_memory_t() : pm));
    this->apply(Y, X, pm);
    return Y;
}
                                           ...
                                           sddk::memory_t pm)
{
    ...
    for (int ichunk = 0; ichunk < bp.num_chunks(); ++ichunk) {
        ...
        sddk::linalg(la).gemm('C', 'N', G.size(1), nbnd, G.size(0), &sddk::linalg_const<numeric_t>::one(), G.at(pm),
                              G.ld(), X.at(pm), X.ld(), &sddk::linalg_const<numeric_t>::zero(),
                              bphi.at(pm, row_offset, 0), bphi.ld());
    }
    assert(num_beta == static_cast<int>(bphi.size(0)) && nbnd == static_cast<int>(bphi.size(1)));

    sddk::linalg_t lapack;
    switch (pu) {
        case sddk::device_t::CPU: {
            lapack = sddk::linalg_t::lapack;
            break;
        }
        case sddk::device_t::GPU: {
            lapack = sddk::linalg_t::gpublas;
            break;
        }
        default:
    ...
    sddk::linalg(lapack).getrs('N', num_beta, nbnd, LU.at(pm), LU.ld(), ipiv.at(pm), bphi.at(pm), bphi.ld());
    ...
    for (int ichunk = 0; ichunk < bp.num_chunks(); ++ichunk) {
        ...
        switch (pu) {
            case sddk::device_t::CPU: {
                sddk::linalg(sddk::linalg_t::blas)
                    .gemm('N', 'N', m, n, k, &sddk::linalg_const<numeric_t>::one(), G.at(sddk::memory_t::host), G.ld(),
                          R.at(sddk::memory_t::host, beta_coeffs.beta_chunk.offset_, 0), R.ld(),
                          &sddk::linalg_const<numeric_t>::one(), Y.at(sddk::memory_t::host), Y.ld());
                break;
            }
#ifdef SIRIUS_GPU
            case sddk::device_t::GPU:
                sddk::linalg(sddk::linalg_t::gpublas)
                    .gemm('N', 'N', m, n, k, &sddk::linalg_const<numeric_t>::one(), G.at(sddk::memory_t::device),
                          G.ld(), R.at(sddk::memory_t::device, beta_coeffs.beta_chunk.offset_, 0), R.ld(),
                          &sddk::linalg_const<numeric_t>::one(), Y.at(sddk::memory_t::device), Y.ld());
