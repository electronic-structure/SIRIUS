    ...
    if (this->approximation_ == 1) {
                    ...
                    for (int is = 0; is < ctx_.num_spins(); is++) {
                        ...
                        for (int m1 = 0; m1 < lmax_at; m1++) {
                            this->hubbard_energy_ +=
                                (atom.type().Hubbard_alpha() + 0.5 * U_effective) * this->occupancy_number_(m1, m1, is, ia, 0).real();
                ...
                if ((std::abs(atom.type().Hubbard_J0()) > 1e-8) || (std::abs(atom.type().Hubbard_beta()) > 1e-8)) {
                    for (int is = 0; is < ctx_.num_spins(); is++) {
                        ...
                        for (int m1 = 0; m1 < lmax_at; m1++) {
                            ...
                            this->hubbard_energy_ += sign * atom.type().Hubbard_beta() * this->occupancy_number_(m1, m1, is, ia, 0).real();

                            this->U(m1, m1, is, ia) += sign * atom.type().Hubbard_beta();

                            for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                                this->hubbard_energy_ +=
                                    0.5 * atom.type().Hubbard_J0() *
                                    (this->occupancy_number_(m2, m1, is, ia, 0) * this->occupancy_number_(m1, m2, s_opposite, ia, 0))
                                        .real();

                                this->U(m1, m2, is, ia) += atom.type().Hubbard_J0() * this->occupancy_number_(m2, m1, s_opposite, ia, 0);
                            }
                        }
                    }
                }
        ...
        for (int ia = 0; ia < this->unit_cell_.num_atoms(); ia++) {
            ...
            this->hubbard_energy_dc_contribution_ +=
                0.5 * (atom.type().Hubbard_U() * n_total * (n_total - 1.0) - atom.type().Hubbard_J() * n_total * (0.5 * n_total - 1.0) -
                    ...
                    for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                        for (int m3 = 0; m3 < 2 * atom.type().hubbard_l() + 1; m3++) {
                            for (int m4 = 0; m4 < 2 * atom.type().hubbard_l() + 1; m4++) {
                                ...
                                if (ctx_.num_mag_dims() == 0) {
                                    this->U(m1, m2, is, ia) +=
                                        2.0 * atom.type().hubbard_matrix(m1, m3, m2, m4) * this->occupancy_number_(m3, m4, is, ia, 0);
                                } else {
                                    // colinear case
                                    for (int is2 = 0; is2 < 2; is2++) {
                                        this->U(m1, m2, is, ia) +=
                                            atom.type().hubbard_matrix(m1, m3, m2, m4) * this->occupancy_number_(m3, m4, is2, ia, 0);
                                    }
                                }

                                this->U(m1, m2, is, ia) -=
                                    atom.type().hubbard_matrix(m1, m3, m4, m2) * this->occupancy_number_(m3, m4, is, ia, 0);

                                this->hubbard_energy_u_ +=
                                    0.5 * ((atom.type().hubbard_matrix(m1, m2, m3, m4) - atom.type().hubbard_matrix(m1, m2, m4, m3)) *
                                               this->occupancy_number_(m1, m3, is, ia, 0) * this->occupancy_number_(m2, m4, is, ia, 0) +
                                           atom.type().hubbard_matrix(m1, m2, m3, m4) * this->occupancy_number_(m1, m3, is, ia, 0) *
...
void Hubbard_potential::calculate_hubbard_potential_and_energy_non_colinear_case()
{
    ...
    for (int ia = 0; ia < unit_cell_.num_atoms(); ia++) {
        ...
        if (atom.type().hubbard_correction()) {
            ...
            mx = n_total.real();
            this->hubbard_energy_dc_contribution_ +=
                0.5 * (atom.type().Hubbard_U() * mx * (mx - 1.0) - atom.type().Hubbard_J() * mx * (0.5 * mx - 1.0) -
                       0.5 * atom.type().Hubbard_J() * magnetization);
            ...
            for (int is = 0; is < 4; is++) {
                            ...
                            for (int m3 = 0; m3 < lmax_at; ++m3) {
                                for (int m4 = 0; m4 < lmax_at; ++m4) {
                                    this->hubbard_energy_flip_ -=
                                        0.5 * (atom.type().hubbard_matrix(m1, m2, m4, m3) * this->occupancy_number_(m1, m3, is, ia, 0) *
                ...
                for (int m1 = 0; m1 < lmax_at; m1++) {
                    ...
                    if (is1 == is) {
                        this->U(m1, m1, is, ia) +=
                            0.5 * (atom.type().Hubbard_U() - atom.type().Hubbard_J()) - atom.type().Hubbard_U() * n_total;
                    }
                    ...
                    for (int m2 = 0; m2 < lmax_at; m2++) {
                        for (int m3 = 0; m3 < lmax_at; m3++) {
                            for (int m4 = 0; m4 < lmax_at; m4++) {
                                this->U(m1, m2, is, ia) -=
                                    atom.type().hubbard_matrix(m1, m3, m4, m2) * this->occupancy_number_(m3, m4, is1, ia, 0);
                            }
                        }
                    }
                }
