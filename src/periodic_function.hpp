    ...
    parameters_(parameters__), num_gvec_(num_gvec)
{
    ...
    f_mt_.set_dimensions(angular_domain_size, parameters_.unit_cell()->max_num_mt_points(), parameters_.unit_cell()->num_atoms());
    f_mt_local_.set_dimensions(parameters_.unit_cell()->spl_num_atoms().local_size());
    ...
    for (int ialoc = 0; ialoc < parameters_.unit_cell()->spl_num_atoms().local_size(); ialoc++)
    {
        ...
        f_mt_local_(ialoc) = new Spheric_function<T>(NULL, angular_domain_size, parameters_.unit_cell()->atom(ia)->radial_grid());
    }
...
{
    for (int ialoc = 0; ialoc < parameters_.unit_cell()->spl_num_atoms().local_size(); ialoc++) delete f_mt_local_(ialoc);
}
...
void Periodic_function<T>::zero()
{
    ...
    for (int ialoc = 0; ialoc < parameters_.unit_cell()->spl_num_atoms().local_size(); ialoc++) f_mt_local_(ialoc)->zero();
    f_it_local_.zero();
}
...
inline void Periodic_function<T>::sync(bool sync_mt, bool sync_it)
{
    ...
    if (f_mt_.get_ptr() != NULL && sync_mt)
    {
        Platform::allgather(&f_mt_(0, 0, 0), 
                            f_mt_.size(0) * f_mt_.size(1) * parameters_.unit_cell()->spl_num_atoms().global_offset(), 
                            f_mt_.size(0) * f_mt_.size(1) * parameters_.unit_cell()->spl_num_atoms().local_size());
    }
}
...
{
    it_val = 0.0;
    mt_val.resize(parameters_.unit_cell()->num_atoms());
    memset(&mt_val[0], 0, parameters_.unit_cell()->num_atoms() * sizeof(T));
    ...
    it_val *= (parameters_.unit_cell()->omega() / fft_->size());
    ...
    for (int ialoc = 0; ialoc < parameters_.unit_cell()->spl_num_atoms().local_size(); ialoc++)
    {
        ...
        Spline<T> s(nmtp, parameters_.unit_cell()->atom(ia)->type()->radial_grid());
        for (int ir = 0; ir < nmtp; ir++) s[ir] = f_mt<local>(0, ir, ialoc);
        mt_val[ia] = s.interpolate().integrate(2) * fourpi * y00;
    }
    
    Platform::allreduce(&mt_val[0], parameters_.unit_cell()->num_atoms());

    T total = it_val;
    for (int ia = 0; ia < parameters_.unit_cell()->num_atoms(); ia++) total += mt_val[ia];
...
size_t Periodic_function<T>::pack(T* array)
{
    ...
    {
        for (int i1 = 0; i1 < f_mt_.size(1); i1++)
        {
            for (int i0 = 0; i0 < f_mt_.size(0); i0++) array[n++] = f_mt_(i0, i1, ia);
        }
    }
...
size_t Periodic_function<T>::unpack(T* array)
{
    ...
    for (int ia = 0; ia < parameters_.unit_cell()->num_atoms(); ia++)
    {
        for (int i1 = 0; i1 < f_mt_.size(1); i1++)
        {
            for (int i0 = 0; i0 < f_mt_.size(0); i0++) f_mt_(i0, i1, ia) = array[n++];
        }
    }
