...
void hubbard_compute_occupation_numbers(K_point_set& kset_)
{
    ...
    for (int ikloc = 0; ikloc < kset_.spl_num_kpoints().local_size(); ikloc++) {
        ...
        // if (nbnd_loc) {
        mdarray<double_complex, 2> dm(this->number_of_hubbard_orbitals(),
                            ...
                            for (int m = 0; m < 2 * atom.type().hubbard_l() + 1; m++) {
                                for (int mp = 0; mp < 2 * atom.type().hubbard_l() + 1; mp++) {
                                    this->occupancy_number_(m, mp, s, ia) +=
                                        conj(dm(this->offset[ia] + m + s1 * (2 * atom.type().hubbard_l() + 1), nband)) *
                                        dm(this->offset[ia] + mp + s2 * (2 * atom.type().hubbard_l() + 1), nband) * kp->band_occupancy(nband) * kp->weight();
                                }
                            }
                        }
                    }
                    ...
                    for(int s = 0; s < 2 * ctx_.num_spins(); s++) {
                        for (int m1 = 0; m1 < 2 * atom.type().hubbard_l() + 1; m1++) {
                            for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                                printf("%.5lf %.5lf ",
                                       this->occupancy_number_(m1, m2, s, ia).real(),
                                       this->occupancy_number_(m1, m2, s, ia).imag());
                            }
                            printf("\n");
                        }
                        printf("\n");
                                    ...
                                    for (int s2 = 0; s2 < ctx_.num_spins(); s2++) {
                                        ...
                                        for (int jj = 0; jj < 2 * atom.type().hubbard_l() + 1; jj++) {
                                            ...
                                            for (int kk = 0; kk < 2 * atom.type().hubbard_l() + 1; kk++) {
                                                ...
                                                rot_spa(2 * s1 + s2) += std::conj(rotm(l1, l3)) *
                                                    occupancy_number_(jj,
                                                                      kk,
                                                                      (s1 == s2) * s1 + (s1 != s2) * (1 + 2 * s1 + s2),
                                                                      ia) * rotm(l2, l4) * alpha;
                                            }
                                        }
                                    }
    ...
    for (int ia = 0; ia < unit_cell_.num_atoms(); ia++) {
        ...
        if (atom.type().hubbard_correction()) {
            for (int s1 = 0; s1 < ctx_.num_spins(); s1++) {
                ...
                for (int m = 0; m < (2 * atom.type().hubbard_l() + 1); m++) {
                    for (int mp = m + 1; mp < (2 * atom.type().hubbard_l() + 1); mp++) {
                        this->occupancy_number_(mp, m, s1, ia) =
                            std::conj(this->occupancy_number_(m, mp, s1, ia));
                    }
                }
            }
            ...
            if(ctx_.num_mag_dims() == 3) {
                ...
                for (int m = 0; m < (2 * atom.type().hubbard_l() + 1); m++) {
                    for (int mp = m + 1; mp < (2 * atom.type().hubbard_l() + 1); mp++) {
                        this->occupancy_number_(mp, m, 2, ia) =
                            std::conj(this->occupancy_number_(m, mp, 3, ia));
                    }
                }
            }
        }
        ...
        for (int ia = 0; ia < unit_cell_.num_atoms(); ia++) {
            ...
            if (atom.type().hubbard_correction()) {
                for(int s = 0; s < 2 * ctx_.num_spins(); s++) {
                    for (int m1 = 0; m1 < 2 * atom.type().hubbard_l() + 1; m1++) {
                        for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                            printf("%.5lf %.5lf ", this->occupancy_number_(m1, m2, s, ia).real(), this->occupancy_number_(m1, m2, s, ia).imag());
                        }
                        printf("\n");
                    }
                    printf("\n");
        ...
        if (atom.type().hubbard_correction()) {
            ...
            if (!nm) {
                if (ctx_.num_mag_dims() != 3) {
                    ...
                    double_complex cs = double_complex(cos(atom.type().magnetization_phi()),
                                                       sin(atom.type().magnetization_phi()));
                    double_complex ns[4];
