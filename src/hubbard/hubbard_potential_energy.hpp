...
// #pragma omp parallel for reduction(+ : hubbard_energy_)
...
//                                 this->occupancy_number_(m1, m1, is, ia).real();
//                             this->U(m1, m1, is, ia) +=
//                                 (atom.type().Hubbard_alpha() + 0.5 * U_effective);

//                             for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {

//                                 this->hubbard_energy_ -= 0.5 * U_effective * (this->occupancy_number_(m1, m2, is, ia) *
//                                                                               this->occupancy_number_(m2, m1, is, ia)).real();

//                                 // POTENTIAL

//                                 this->U(m1, m2, is, ia) -=
//                                     U_effective * this->occupancy_number_(m2, m1, is, ia);
//                             }
//                         }
//                     }
//                 }
...
//                         int s_opposite = ctx_.num_spins() - 1 - is;
//                         double sign    = (is == 0) - (is == 1);

//                         for (int m1 = 0; m1 < 2 * atom.type().hubbard_l() + 1; m1++) {

//                             this->hubbard_energy_ +=
//                                 sign * atom.type().Hubbard_beta() * this->occupancy_number_(m1, m1, is, ia).real();
...
//                                      this->occupancy_number_(m1, m2, s_opposite, ia)).real();

//                                 this->U(m1, m2, is, ia) +=
//                                     atom.type().Hubbard_J0() * this->occupancy_number_(m2, m1, s_opposite, ia);
//                             }
//                         }
//                     }
//                 }
...
//     } else {
        ...
        //        #pragma omp parallel for reduction(+ : hubbard_energy_dc_contribution_) reduction(+ : hubbard_energy_u_)
        for (int ia = 0; ia < this->unit_cell_.num_atoms(); ia++) {
            ...
            if (ctx_.num_spins() == 1) {
                ...
                for (int m = 0; m < 2 * atom.type().hubbard_l() + 1; m++) {
                    magnetization += (this->occupancy_number_(m, m, 0, ia) -
                                      this->occupancy_number_(m, m, 1, ia)).real();
                }
                magnetization *= magnetization;
            }

            this->hubbard_energy_dc_contribution_ +=
                0.5 * (atom.type().Hubbard_U() * n_total * (n_total - 1.0) -
                    ...
                    for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                        for (int m3 = 0; m3 < 2 * atom.type().hubbard_l() + 1; m3++) {
                            for (int m4 = 0; m4 < 2 * atom.type().hubbard_l() + 1; m4++) {
                                ...
                                this->U(m1, m2, is, ia) -=
                                    atom.type().hubbard_matrix(m1, m3, m4, m2) * this->occupancy_number_(m3, m4, is, ia);
                                ...
                                this->hubbard_energy_u_ += 0.5*
                                    ((atom.type().hubbard_matrix(m1, m3, m2, m4) -
                                     ...
                                     atom.type().hubbard_matrix(m1, m3, m2, m4) *
                                     this->occupancy_number_(m1, m2, is, ia) *
                                     this->occupancy_number_(m3, m4, (is + 1)%2, ia)
                                     ).real();
    ...
    if(ctx_.control().verbosity_ > 2) {
        printf("hub Energy %.5lf %.5lf %.5lf %.5lf\n",
               this->hubbard_energy_,
               this->hubbard_energy_noflip_,
               this->hubbard_energy_flip_,
               this->hubbard_energy_dc_contribution_);
        printf("hubbard potential -------------------------------------------------\n");
        for (int ia = 0; ia < unit_cell_.num_atoms(); ia++) {
            const auto &atom = unit_cell_.atom(ia);
            if (atom.type().hubbard_correction()) {
                for(int s = 0; s < 2 * ctx_.num_spins(); s++) {
                    for (int m1 = 0; m1 < 2 * atom.type().hubbard_l() + 1; m1++) {
                        for (int m2 = 0; m2 < 2 * atom.type().hubbard_l() + 1; m2++) {
                            printf("%.5lf %.5lf ", this->U(m1, m2, s, ia).real(), this->U(m1, m2, s, ia).imag());
                        }
                        printf("\n");
                    }
                    printf("\n");
...
void calculate_hubbard_potential_and_energy_non_colinear_case()
{
    ...
    for (int ia = 0; ia < unit_cell_.num_atoms(); ia++) {
        ...
        if (atom.type().hubbard_correction()) {
            ...
            n_total = (this->occupancy_number_(0, 0, 0, ia) + this->occupancy_number_(0, 0, 1, ia)).real();
            ...
            for (int m = 1; m < 2 * atom.type().hubbard_l() + 1; m++) {
                n_total +=
                    (this->occupancy_number_(m, m, 0, ia) + this->occupancy_number_(m, m, 1, ia)).real();
            ...
            for (int is = 0; is < 4; is++) {
                                ...
                                for (int m4 = 0; m4 < 2 * atom.type().hubbard_l() + 1; ++m4) {
                                    ...
                                    this->hubbard_energy_noflip_ +=
                                        ((atom.type().hubbard_matrix(m1, m3, m2, m4) -
                                         ...
                                         atom.type().hubbard_matrix(m1, m3, m2, m4) *
                                         this->occupancy_number_(m1, m2, is, ia) *
                                         this->occupancy_number_(m3, m4, (is + 1)%2, ia)).real();
                                }
                            }
                        }
                    }
                            ...
                            for (int m3 = 0; m3 < 2 * atom.type().hubbard_l() + 1; ++m3) {
                                for (int m4 = 0; m4 < 2 * atom.type().hubbard_l() + 1; ++m4) {
                                    this->hubbard_energy_flip_ -= (atom.type().hubbard_matrix(m1, m3, m2, m4) *
                                                                   this->occupancy_number_(m1, m2, is, ia) *
                                                                   this->occupancy_number_(m3, m4, is1, ia)).real();
                                }
                            }
                        }
                    }
                ...
                double n_aux = 0.0;
                for (int m1 = 0; m1 < 2 * atom.type().hubbard_l() + 1; m1++) {
                    n_aux += this->occupancy_number_(m1, m1, is1, ia).real();
                }
    ...
    if(ctx_.control().verbosity_ > 2) {
        printf("hub Energy (t) %.15lf (no) %.15lf (flip) %.15lf (dc) %.15lf\n",
               this->hubbard_energy_,
               this->hubbard_energy_noflip_,
               this->hubbard_energy_flip_,
               this->hubbard_energy_dc_contribution_);
