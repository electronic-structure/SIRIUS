...
void apply_hubbard_potential(const K_point& kp, const int idx__, const int n__, Wave_functions& phi, Wave_functions& ophi)
{
    ...
    for (int s = 1; s < ctx_.num_spins(); s++) {
        linalg<CPU>::gemm(2, 0,
                          this->number_of_hubbard_orbitals(),
                          n__,
                          kp.hubbard_wave_functions_ppus(s).pw_coeffs().num_rows_loc(),
