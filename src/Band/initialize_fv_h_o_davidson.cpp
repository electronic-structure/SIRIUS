...
template<> 
void Band::initialize_fv_h_o_davidson<CPU, full_potential_lapwlo>(K_point* kp__,
                                                  Periodic_function<double>* effective_potential__,
                                                  dmatrix<double_complex>& h__,
                                                  dmatrix<double_complex>& o__) const
{
    PROFILE_WITH_TIMER("sirius::Band::initialize_fv_h_o_davidson");
    ...
    runtime::Timer t1("sirius::Band::initialize_fv_h_o_davidson|zgemm");
        ...
        #pragma omp parallel
        {
            ...
            for (int ia = iblk * num_atoms_in_block; ia < std::min(unit_cell_.num_atoms(), (iblk + 1) * num_atoms_in_block); ia++)
            {
                if (ia % omp_get_num_threads() == tid)
                {
                    ...
                    initialize_fv_h_o_apw_lo_davidson(kp__, type, atom, ia, alm_row_tmp, alm_col_tmp, h__.panel(), o__.panel());
                }
            }
        }
        #ifdef __PRINT_OBJECT_CHECKSUM
    ...
    initialize_fv_h_o_it_davidson(kp__, effective_potential__, h__.panel(), o__.panel());

    /* setup lo-lo block */
    initialize_fv_h_o_lo_lo_davidson(kp__, h__.panel(), o__.panel());
}
...
void Band::initialize_fv_h_o_apw_lo_davidson(K_point* kp, 
...
void Band::initialize_fv_h_o_it_davidson(K_point* kp, Periodic_function<double>* effective_potential, 
...
void Band::initialize_fv_h_o_lo_lo_davidson(K_point* kp, mdarray<double_complex, 2>& h, mdarray<double_complex, 2>& o) const
{
