                                                    ...
                                                    std::vector<double>& veff_it_coarse__)
{
    ...
    Wave_functions ophi(num_phi, kp__->gkvec(), ctx_.mpi_grid_fft(), true);

    Wave_functions hpsi(num_bands, kp__->gkvec(), ctx_.mpi_grid_fft(), true);
    Wave_functions opsi(num_bands, kp__->gkvec(), ctx_.mpi_grid_fft(), true);
    ...
    Wave_functions res(num_bands, kp__->gkvec(), ctx_.mpi_grid_fft(), true);

    /* norm of residuals */
    std::vector<double> res_norm(num_bands);

    /* offset in the packed array of on-site matrices */ // TODO: isolate in a separate object
    ...
    std::memcpy(&phi(0, 0), &psi(0, 0), ngk * num_bands * sizeof(double_complex));
    ...
    {
        /* a pply Hamiltonian and overlap operators to the new basis functions */
        STOP();
