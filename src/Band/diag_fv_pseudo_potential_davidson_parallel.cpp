...
//==     auto& psi_slab = kp__->fv_states()->coeffs();
//== 
//==     ///* temporary buffer */
...
//==     
//==     dmatrix<double_complex>     hmlt(num_phi, num_phi, kp__->blacs_grid(), bs, bs);
//==     dmatrix<double_complex>     ovlp(num_phi, num_phi, kp__->blacs_grid(), bs, bs);
//==     dmatrix<double_complex> hmlt_old(num_phi, num_phi, kp__->blacs_grid(), bs, bs);
//==     dmatrix<double_complex> ovlp_old(num_phi, num_phi, kp__->blacs_grid(), bs, bs);
//== 
//==     dmatrix<double_complex> evec(num_phi, num_bands, kp__->blacs_grid(), bs, bs);
//== 
...
//==     std::memcpy(&phi(0, 0), &psi_slab(0, 0), num_gkvec_loc * num_bands * sizeof(double_complex));
//== 
...
//==         //            if (kp__->band_occupancy(i) > 1e-10 && res_norm[i] > ctx_.iterative_solver_tolerance())
//==         //            {
