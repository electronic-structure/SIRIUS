...
def main():
    parser = argparse.ArgumentParser(description='Generate the Fortran API from the given FILE (usually the sirius_api.cpp)', formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument("FILE", type=argparse.FileType('r'))
    parser.add_argument('-o', '--output', type=argparse.FileType('w'), default='generated.f90', help='The output file name')
    ...
    nfunc = 0
    for r in re.findall('(@api\s+begin\s+((\w|\W|\s)*?)@api\s+end)+', args.FILE.read()):
