                                      ...
                                      Wave_functions* ophi__) const
{
    ...
    for (int iblk = 0; iblk < nblk; iblk++) {
        ...
        if (!apw_only__) {
            ...
            utils::timer t3("sirius::Band::apply_fv_h_o|lo*");
            /* lo-APW contribution */
            for (int ia = atom_begin; ia < atom_end; ia++) {
                int ialoc = ia - atom_begin;
                auto& atom = unit_cell_.atom(ia);
                            ...
                            if (lm_lo == lm1 && ophi__ != nullptr) {
                                for (int i = 0; i < n__; i++) {
                                    ophi__->mt_coeffs(0).prime(ophi__->offset_mt_coeffs(ia_location.local_index) + ilo, N__ + i) +=
                                        phi_lo_block(offsets_lo[ialoc] + jlo, i) * atom.symmetry_class().o_radial_integral(l_lo, order_lo, order1);
                                }
                            }
                            if (hphi__ != nullptr) {
                                ...
                                for (int i = 0; i < n__; i++) {
                                    hphi__->mt_coeffs(0).prime(hphi__->offset_mt_coeffs(ia_location.local_index) + ilo, N__ + i) +=
                                        phi_lo_block(offsets_lo[ialoc] + jlo, i) * atom.radial_integrals_sum_L3<spin_block_t::nm>(idxrf_lo, idxrf1, gc);
                                }
                            }
                        }
                            ...
                            if (hphi__ != nullptr) {
                                for (int i = 0; i < n__; i++) {
                                    ...
                                    for (int xi = 0; xi < type.mt_aw_basis_size(); xi++) {
                                        ...
                                        z += atom.radial_integrals_sum_L3<spin_block_t::nm>(idxrf_lo, idxrf_aw, gc) * alm_phi(offsets_aw[ialoc] + xi, i);
                                    }
...
//= 
//=     /* interstitial part */
//=     local_op_->apply_h_o(N__, n__, phi__, nullptr, &ophi__);
//= 
//= #ifdef __GPU
//=     if (ctx_.processing_unit() == GPU) {
