...
Hamiltonian_k<T>::set_fv_h_o_it(int ispn__, la::dmatrix<std::complex<T>>& h__, la::dmatrix<std::complex<T>>& o__) const
{
    ...
    for (int igk_col = 0; igk_col < kp.num_gkvec_col(); igk_col++) {
        ...
        for (int igk_row = 0; igk_row < kp.num_gkvec_row(); igk_row++) {
            ...
            h__(igk_row, igk_col) += H0().veff_pw(ig12, ispn__);
            ...
            switch (H0().ctx().valence_relativity()) {
                case relativity_t::iora: {
                    o__(igk_row, igk_col) += t1 * sq_alpha_half * H0().rm2_inv_pw(ig12);
                }
                case relativity_t::zora: {
                    h__(igk_row, igk_col) += t1 * H0().rm_inv_pw(ig12);
                    break;
                }
                default: {
                               ...
                               wf::Wave_functions<T>* hphi__, wf::Wave_functions<T>* ophi__) const
{
     ...
     * eigen-value solver */
    auto& blacs_grid = ctx.blacs_grid_band();
    ...
    auto apply_omt_apw_lo = [this, &ctx, &phi__, &b__, &spl_atoms, nat_loc](wf::Wave_functions_mt<T>& o_apw_lo__) {
        ...
        for (int ialoc = 0; ialoc < nat_loc; ialoc++) {
            ...
            auto const& rb = this->H0().lapw_basis().radial_basis(ia);
    ...
    /* precompute temporary objects (MT hamiltonian / overlap x lo part of wave-functions) that
     * are later used to compute APW-lo contribution; the application of the Hamiltonian and
     * overlap is local to the MPI rank; afterwards the data is reshuffled to 2D scalapack distribution 
     * with COSTA to be later used in spla call */
