#ifndef NON_LOCAL_OPERATOR_IMPL_H
#define NON_LOCAL_OPERATOR_IMPL_H
...
class Non_local_operator
{
  ...
  public:
    ...
    apply(sddk::memory_t mem__, int chunk__, int ispn_block__, wf::Wave_functions<T>& op_phi__, wf::band_range br__,
          beta_projectors_coeffs_t<T> const& beta_coeffs__, sddk::matrix<F> const& beta_phi__) const
    {
                               ...
                               identity_t<F> alpha, identity_t<F> beta) const
{
    ...
    for (int ja = 0; ja < num_atoms; ++ja) {
        ...
        int nbf = size_ja;
        // compute Bij * Qjj
        la::wrap(la).gemm('N', 'N', B__.size(0), size_ja, size_ja, &alpha, Bs, B__.ld(), Qs, nbf, &beta, C,
                          out.ld());
        }
}
...
#endif /* NON_LOCAL_OPERATOR_IMPL_H */
