...
namespace sddk {
    ...
    dmatrix<T>::dmatrix(int num_rows__, int num_cols__, BLACS_grid const &blacs_grid__, int bs_row__, int bs_col__,
            ...
            splindex<splindex_t::block_cyclic>(num_cols__, blacs_grid__.num_ranks_col(), blacs_grid__.rank_col(),
                                               bs_col__).local_size(),
            mem_type__), num_rows_(num_rows__), num_cols_(num_cols__), bs_row_(bs_row__), bs_col_(bs_col__),
    ...
    dmatrix<T>::dmatrix(int num_rows__, int num_cols__, memory_t mem_type__)
            : matrix<T>(num_rows__, num_cols__, mem_type__), num_rows_(num_rows__), num_cols_(num_cols__), bs_row_(1),
              bs_col_(1), spl_row_(num_rows_, 1, 0, bs_row_), spl_col_(num_cols_, 1, 0, bs_col_) {
    }
    ...
    dmatrix<T>::dmatrix(T *ptr__, int num_rows__, int num_cols__, BLACS_grid const &blacs_grid__, int bs_row__,
                        int bs_col__)
            : matrix<T>(ptr__,
                        splindex<splindex_t::block_cyclic>(num_rows__, blacs_grid__.num_ranks_row(),
                                                           blacs_grid__.rank_row(), bs_row__).local_size(),
                        splindex<splindex_t::block_cyclic>(num_cols__, blacs_grid__.num_ranks_col(),
                                                           blacs_grid__.rank_col(), bs_col__).local_size()),
    ...
    dmatrix<T>::dmatrix(T *ptr__, int num_rows__, int num_cols__)
            : matrix<T>(ptr__, num_rows__, num_cols__), num_rows_(num_rows__), num_cols_(num_cols__), bs_row_(1),
              bs_col_(1), spl_row_(num_rows_, 1, 0, bs_row_), spl_col_(num_cols_, 1, 0, bs_col_) {
        init();
    }
    ...
    void dmatrix<T>::set(int ir0__, int jc0__, int mr__, int nc__, T *ptr__, int ld__) {
        ...
        splindex<splindex_t::block_cyclic> spl_c0(jc0__, blacs_grid().num_ranks_col(), blacs_grid().rank_col(),
                                                  bs_col_);
    ...
    // instantiate class for required types
