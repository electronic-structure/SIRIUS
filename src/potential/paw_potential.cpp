...
Potential::generate_PAW_effective_potential(Density const& density)
{
    ...
    sirius::symmetrize_mt_function(unit_cell_, ctx_.rotm_rlm(), ctx_.mpi_grid_mt_sym(), ctx_.num_mag_dims(), ae_comp);
    sirius::symmetrize_mt_function(unit_cell_, ctx_.rotm_rlm(), ctx_.mpi_grid_mt_sym(), ctx_.num_mag_dims(), ps_comp);

    /* symmetrize ae- component of Exc */
    paw_ae_exc_->sync(unit_cell_.spl_num_paw_atoms());
    ae_comp.clear();
    ae_comp.push_back(paw_ae_exc_.get());
    sirius::symmetrize_mt_function(unit_cell_, ctx_.rotm_rlm(), ctx_.mpi_grid_mt_sym(), 0, ae_comp);
