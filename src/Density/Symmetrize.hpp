                ...
                const bool hubbard_)
{
    ...
    for (int xi1 = 0; xi1 < indexb.size(); xi1++) {
        lmax = std::max(lmax,  indexb[xi1].l);
    }

    mdarray<double_complex, 3> dm1_(2 * lmax + 1, 2 * lmax + 1, ctx_.num_spins() * ctx_.num_spins());
    mdarray<double_complex, 3> dm2_(2 * lmax + 1, 2 * lmax + 1, ctx_.num_spins() * ctx_.num_spins());
    ...
    for (int xi1 = 0; xi1 < indexb.size(); xi1++) {
        ...
        for (int xi2 = 0; xi2 < indexb.size(); xi2++) {
            ...
            if (l1 == l2) {
                /* the rotation matrix of the angular momentum is block diagonal
                   and does not couple different l.
                */

                /* collect the block diagonal part */
                for (int j = 0; j < ndm; j++) {
                    for (int m3 = -l1; m3 <= l1; m3++) {
                        ...
                        for (int m4 = -l2; m4 <= l2; m4++) {
                            ...
                            dm1_(lm3, lm4, j) = ns_(xi3, xi4, j, ja);
                        }
                    }
                }

                apply_symmetry(dm1_, rotm, spin_rot_su2, ndm, l2, dm2_);
                ...
                for (int j = 0; j < ndm; j++) {
                    for (int m3 = -l1; m3 <= l1; m3++) {
                        ...
                        for (int m4 = -l2; m4 <= l2; m4++) {
                            int lm4 = utils::lm(l2, m4);
                            dm_(xi1 + lm3, xi2 + lm4, j, ia) += dm2_(lm3, lm4, j);
                        }
                    }
                }
            }
