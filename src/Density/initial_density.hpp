                        ...
                        for (int t0 = -1; t0 <= 1; t0++) {
                            for (int t1 = -1; t1 <= 1; t1++) {
                                for (int t2 = -1; t2 <= 1; t2++) {
                                    ...
                                    if (a <= R) {
                                        magnetization_[0]->f_rg(ir) += v[2] * 1.0 / (std::exp( (a-R)/0.1) + 1.0) / norm;
                                        //magnetization_[0]->f_rg(ir) += v[2] * (1.0 - a / R) / norm;
                                        if (ctx_.num_mag_dims() == 3) {
                                            magnetization_[1]->f_rg(ir) += v[0] * 1.0 / (std::exp( (a-R)/0.1) + 1.0) / norm;
                                            magnetization_[2]->f_rg(ir) += v[1] * 1.0 / (std::exp( (a-R)/0.1) + 1.0) / norm;
